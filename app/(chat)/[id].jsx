...
const ChatScreen = () => {
  ...
  const [groupMembers, setGroupMembers] = useState([]);
  const [currentUser, setCurrentUser] = useState(null);
  const socketRef = useRef(null); // Sử dụng useRef thay vì useState
  const scrollViewRef = useRef(null); // Thêm ref để cuộn tự động
  ...
  useEffect(() => {
    const initializeSocket = async () => {
      try {
        ...
        // Khởi tạo Socket.IO với auth đồng bộ và query
        socketRef.current = io(API_BASE, {
          transports: ['websocket'],
          auth: { token: `Bearer ${token}` }, // Token đồng bộ
          query: { userId }, // Gửi userId để server nhận
        });

        socketRef.current.on('connect', () => {
          console.log('Kết nối Socket.IO thành công:', socketRef.current.id);
          socketRef.current.emit('joinGroup', { groupId, userId });
          socketRef.current.emit('requestGroupMembers', groupId);
        });

        socketRef.current.on('newGroupMessage', (message) => {
          console.log('Tin nhắn nhóm mới:', message);
          AsyncStorage.getItem('userId').then((userId) => {
            ...
            setMessages((prev) => {
              if (prev.some((msg) => msg.id === formattedMessage.id)) {
                console.log(`Tin nhắn ${formattedMessage.id} đã tồn tại, bỏ qua`);
                return prev;
              }
              console.log(`Thêm tin nhắn mới: ${formattedMessage.id}`);
              return [...prev, formattedMessage];
            });
            scrollViewRef.current?.scrollToEnd({ animated: true }); // Cuộn tự động
          });
        });

        socketRef.current.on('groupMembersUpdated', (members) => {
          const formattedMembers = members.map((member) => {
            console.log(`Member ID: ${member._id}, Username: ${member.username}, Avatar: ${member.avatar || 'none'}`);
            return {
              id: member._id,
        ...
        socketRef.current.on('messageDeleted', (messageId) => {
          console.log('Tin nhắn nhóm đã xóa:', messageId);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Chỉ gửi qua Socket.IO
      console.log(`Gửi tin nhắn: ${JSON.stringify(messageData)}`);
  ...
  const handleDelete = async (messageId) => {
    try {
      ...
      socketRef.current.emit('deleteGroupMessage', { messageId, senderId: userId, groupId });

      const res = await fetch(`${API_BASE}/api/v1/chat/group/${messageId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
